...
import { queryPublicPosts } from 'applet-apis'
...
const loadPublicPosts = createAsyncThunk('post/loadPublicPosts', async (args, thunkAPI) => {
  const stateData: any = thunkAPI.getState()
  const { currentPage, contentTypes } = stateData.publicPost
  const queryParams: any = {
    page: currentPage,
    contentTypes
  }
  const response = await queryPublicPosts(queryParams)
  return response
})
...
const publicPostSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(loadPublicPosts.pending, (state, action) => {
      state.loadingPosts = true
    })
    builder.addCase(loadPublicPosts.fulfilled, (state, action) => {
